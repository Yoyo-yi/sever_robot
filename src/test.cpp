...
//路径点存储绝对路径
#define POINT_FILE_PATH    "//home//cy//python//test2//coordinates.txt"
...
int GetUserInitData(user_setting& setting_data) {
    ...
    setting_data.mode = qt_socket_key.at("mode").get<int>();
...
void BotControl(std::atomic<bool> &stopFlag){
...
void ActionStatus(int polling_time)
{
    usleep(500000);
    while (1) {
...
int TimeMode(const user_setting& setting_data) {
    ...
    while (1) {
        ...
        if (is_within_time_range(now_tm.tm_hour, now_tm.tm_min, setting_data.start_time[0], setting_data.start_time[1], setting_data.end_time[0], setting_data.end_time[1])) {
            ...
            HTTP_RETURN = BotOrder(POST_MOVE, HTTP_OD::BOT_POST, CreateSeriesActionPoint(patrol_points));
            ActionStatus(2000);
            //Sleep(50000);
...
int PowerMode(const user_setting& setting_data) {
...
int main(void)
{
    ...
    patrol_points = get_point_from_file(POINT_FILE_PATH);
